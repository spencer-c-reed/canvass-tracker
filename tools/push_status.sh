...
# Commit only if public dashboard data changed
if ! git diff --quiet status.json tools/corpus_history.json 2>/dev/null; then
  git add status.json tools/corpus_history.json
